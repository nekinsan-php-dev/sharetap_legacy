...
class VcardController extends AppBaseController
{
    ...
    public function show($alias, $id = null): \Illuminate\View\View
    {
        ...
        if ($sharetapPermissions->basic_info == 0){
            unset($vcard->name);
            unset($vcard->first_name);
            unset($vcard->last_name);
            unset($vcard->occupation);
            unset($vcard->email);
            unset($vcard->alternative_email);
            unset($vcard->phone);
            unset($vcard->alternative_phone);
            unset($vcard->company);
            unset($vcard->description);
            unset($vcard->location);
            unset($vcard->dob);
        }
