...
function getSocialLink($vcard): array
{
    ...
    $sharetapPermissions = SharetapPermission::where('vcard_id',$vcard->id)->first();

    if ($sharetapPermissions->website == 0){
        unset($vcardSocials['website']);
    }

    if ($sharetapPermissions->facebook == 0){
        unset($vcardSocials['facebook']);
    }
    if ($sharetapPermissions->twitter == 0){
        unset($vcardSocials['twitter']);
    }
    if ($sharetapPermissions->instagram == 0){
        unset($vcardSocials['instagram']);
    }
    if ($sharetapPermissions->whatsapp == 0){
        unset($vcardSocials['whatsapp']);
    }
    if ($sharetapPermissions->youtube == 0){
        unset($vcardSocials['youtube']);
    }
    if ($sharetapPermissions->linkedin == 0){
        unset($vcardSocials['linkedin']);
    }

    unset($vcardSocials['map']);
